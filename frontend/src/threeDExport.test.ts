...
const ready = { isExporting: false, hasModel: true, webGLAvailable: true, wasmActive: true, hasWasmGeometry: true, rendererMounted: true, rendererGeneration: 'r4', geometryGeneration: 'r4', frameGeneration: 'r4', canonicalGeneration: 'r4' }
describe('3D export admission', () => {
  it('requires current WASM geometry and its mounted renderer', () => {
    ...
    expect(canExportCurrentThreeD({ ...ready, frameGeneration: null })).toBe(false)
    expect(canExportCurrentThreeD({ ...ready, frameGeneration: 'r3' })).toBe(false)
