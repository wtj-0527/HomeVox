...
export function SourceImportView({ selectedFile, previewURL, onFileChange, status, error }: SourceImportViewProps) {
...
export function CropConfirmView({ selectedFile, previewURL, imageSize, detection, crop, status, error, onCropChange, onSelectCandidate, onRestoreRecommended, onResetFullImage, onConfirm }: CropConfirmViewProps) {
  ...
  return <section className="import-workspace mx-auto grid w-full max-w-[1124px] grid-cols-[minmax(0,700px)_396px] gap-7 text-slate-800">
    <div className="workspace-card h-[780px] p-7">
      <div className="flex items-center justify-between"><div><h3 className="text-xl font-bold">确认户型区域</h3><p className="mt-1 text-sm text-slate-500">拖动裁切框或拖动四角和边缘，按方向键可微调。</p></div><span className="rounded-full bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700">原始像素</span></div>
      <svg tabIndex={0} aria-label="户型裁切区域" className="crop-canvas mt-5" viewBox={`0 0 ${imageSize.width} ${imageSize.height}`} onPointerDown={pointer} onPointerMove={pointer} onKeyDown={(event) => { const next = cropKeyboardNudge(crop, event.key, imageSize, event.shiftKey ? 10 : 1); if (next) { event.preventDefault(); onCropChange(next) } }}>
    ...
    <aside className="workspace-card h-[780px] p-7"><h3 className="text-xl font-bold">选择后再继续</h3><p className="mt-2 text-sm leading-6 text-slate-500">只会把当前框选区域发送给 AI 识别。</p>
      {detection?.candidates.length ? <div className="mt-6 space-y-2"><p className="text-sm font-semibold">推荐区域</p>{detection.candidates.map((candidate, index) => <button key={`${candidate.x}-${candidate.y}`} className="crop-candidate" type="button" onClick={() => onSelectCandidate(index)}>候选 {index + 1} · {candidate.width} × {candidate.height}</button>)}</div> : <p className="mt-6 rounded-xl bg-amber-50 p-4 text-sm leading-6 text-amber-800">无法可靠判断户型区域，请手动框选；默认已保留整张原图。</p>}
      <div className="mt-6 grid grid-cols-2 gap-3"><button className="rounded-[10px] border border-violet-300 px-3 py-2 text-sm font-semibold text-violet-700" type="button" onClick={onRestoreRecommended}>恢复推荐</button><button className="rounded-[10px] border border-slate-300 px-3 py-2 text-sm font-semibold text-slate-700" type="button" onClick={onResetFullImage}>全图裁切</button></div>
      {error && <p role="alert" className="mt-4 rounded-[10px] bg-red-50 p-3 text-sm text-red-700">{error}</p>}<button className="mt-7 w-full rounded-[10px] bg-violet-600 px-4 py-3 text-sm font-semibold text-white disabled:opacity-50" type="button" disabled={status === 'uploading'} onClick={onConfirm}>{status === 'uploading' ? '正在判断…' : '确认裁切并判断'}</button></aside>
  </section>
}
