...
const (
	...
	visionUser                 = "Parse this floor-plan image into the required JSON structure. The original uploaded image is exactly %d pixels wide and %d pixels high. Use that exact original %d x %d pixel coordinate grid for every room bound, wall endpoint, and opening width, and return metadata.image_width exactly %d and metadata.image_height exactly %d. Do not include markdown fences. Omit an opening if its wall-local position or width cannot be established."
...
func validContract(request chatRequest) (string, bool) {
	...
	expectedPrompt := fmt.Sprintf(visionUser, 560, 400, 560, 400, 560, 400)
	if json.Unmarshal(request.Messages[1].Content, &user) != nil || user[0].Text != expectedPrompt || !strings.HasPrefix(imageURL, "data:image/png;base64,") {
		return "", false
	}
	return "parse", true
}
